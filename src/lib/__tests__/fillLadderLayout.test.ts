...
describe('layoutFillLadder — role split + free-space (piece 2)', () => {
  ...
  it('falls back to leaderMax when a side is fully blocked (no crash, no infinite loop)', () => {
    const wall: OccupancyRect = { x: 300, y: 0, w: 600, h: 400 }
    const placed = layoutFillLadder([exit(300, 200)], { ...OPTS, candleRects: [wall] })
    expect(placed[0].pillX).toBe(300 + OPTS.leaderMax + OPTS.pillWidth / 2)
  })
