...
//
// v0.2.2 — smoke-found 2026-05-30: a bare decimal like "1.5" (no K/M/B
// suffix) used to silently round to 2 shares via Math.round, corrupting
// user intent (almost always a forgotten-suffix typo). The
// integer-when-no-suffix guard below rejects those; integers like "850"
// and any suffixed decimal like "1.5M" still parse normally.
export function parseInput(raw: string): number | null {
  ...
  // Bare-decimal honesty: a fractional value with no K/M/B suffix isn't a
  // meaningful share count (you can't own 1.5 shares; share counts are
  // integers). Reject it so the FloatEditor's commit() falls through its
  // null path rather than saving a Math.round-corrupted value like 2.
  if (mult === 1 && !Number.isInteger(num)) return null
