...
interface ConfluenceTagsProps {
  trade: TradeListRow
  /** Beat 3 — render for embedding inside another card (the Setup card). Drops
   *  the standalone card shell and the descriptive blurb; keeps the "Confluence"
   *  sub-label, chip row, and Add dropdown, and leads with a hairline divider so
   *  the separation disappears together with the section when the null-gate
   *  fires. Default false → unchanged standalone card. */
  embedded?: boolean
}
...
export default function ConfluenceTags({ trade, embedded = false }: ConfluenceTagsProps) {
  ...
  // Embedded (Beat 3 Setup card): no card shell, no blurb. A leading hairline
  // divider separates these secondary tags from the Playbook control above;
  // because the component returns null when primaryIsSystem (gate above), the
  // divider + label vanish together — no orphaned rule in the Setup card.
  if (embedded) {
    return (
