...
interface AverageTradePnlChartProps {
  daily: DailyPnlPoint[]
}

function AverageTradePnlChartBase({ daily }: AverageTradePnlChartProps) {
    ...
    <div className="h-[180px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 10, right: 8, left: 0, bottom: 0 }}>
          ...
          />
          <Bar dataKey="avg_trade_pnl" radius={[3, 3, 0, 0]} isAnimationActive={animate}>
            {data.map((d) => (
              <Cell key={d.date} fill={d.avg_trade_pnl >= 0 ? palette.win : palette.loss} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
...
export default memo(AverageTradePnlChartBase)
...
function ChartTooltip({
  ...
  active?: boolean
  payload?: { payload: DailyPnlPoint & { label: string } }[]
}) {
  if (!active || !payload?.length) return null
  const d = payload[0].payload
  const positive = d.avg_trade_pnl >= 0
  return (
    <div className="rounded-md border border-border bg-bg-4 px-3 py-2 shadow-md">
      ...
      <div className={`mt-1 font-mono text-sm font-semibold tnum ${positive ? 'text-win' : 'text-loss'}`}>
        {signed(d.avg_trade_pnl)}
        <span className="ml-1 text-[11px] font-normal text-fg-tertiary">/ trade</span>
      </div>
      <div className="mt-1 font-mono text-[11px] text-fg-tertiary tnum">
        {d.trade_count} {d.trade_count === 1 ? 'trade' : 'trades'}{' · '}
        day {signed(d.net_pnl)}
      </div>
    </div>
  )
}
