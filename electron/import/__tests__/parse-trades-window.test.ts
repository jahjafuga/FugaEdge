...
describe('detectFormat routing — trades_window', () => {
  it('routes Brendan-shape CSV to "trades_window"', () => {
...
const BRENDAN_FIXTURE = resolve(
  __dirname,
  '../../../test-fixtures/Brendan Trades Example.csv',
)

describe('Brendan real fixture — generic invariants only', () => {
  if (!existsSync(BRENDAN_FIXTURE)) {
    it.skip('skipped: fixture not present', () => {})
    return
  }

  const csv = readFileSync(BRENDAN_FIXTURE, 'utf8')
